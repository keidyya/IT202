...
if (isset($_POST["join"])) {
    ...
    $stmt = $db->prepare("select fee from Comps where id = :id && expires > current_timestamp && paid_out = 0");
...
$stmt = $db->prepare("SELECT c.*, UC.user_id as reg FROM Comps c LEFT JOIN (SELECT * FROM F20_UserCompetitions where user_id = :id) as UC on c.id = UC.competition_id WHERE c.expires > current_timestamp AND paid_out = 0 AND (UC.user_id = :id OR c.user_id = :id) ORDER BY expires ASC");
